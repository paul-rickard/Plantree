...
| `code`, `name` | |
| *`workOrderTypeCode`* | The [`WorkOrderType`](#workordertype) work generated from this plan is raised as. Not all planned, recurring work is preventive — a plan may drop administrative (`AD`) or operations (`OP`) work (e.g. a compliance return, an operator round). Defaults to `PM`. |
...
| FK `workOrderTypeCode` | The kind of work, referencing a `WorkOrderType` by `code`. Seeded set: `PM \| RM \| AD \| OP`; organisations may add their own. Was a fixed enum; now a configurable catalogue. Schedule-generated work inherits the [`JobPlan.workOrderTypeCode`](#jobplan-versioned); requested work is set on triage. |
| `priority`, `dueDate` | |
